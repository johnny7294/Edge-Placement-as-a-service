...
output = subprocess.Popen("sudo docker exec "+str(container_name)+" ip link set "+veth_pair_name +" up" ,shell=True, stdout=subprocess.PIPE)
(out, err) = output.communicate()
#add ip
output = subprocess.Popen("sudo docker exec "+str(container_name)+" ip addr add "+str(ip_a)+" dev "+veth_pair_name ,shell=True, stdout=subprocess.PIPE)
(out, err) = output.communicate()